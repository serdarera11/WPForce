...
def slice_list(input, size):
    ...
    slice_size = input_size // size  # Use integer division (//) in Python 3
    ...
    for i in range(size):
        ...
        for j in range(slice_size):
            result[i].append(next(iterator))  # Use next() instead of iterator.next()
        if remain:
            result[i].append(next(iterator))  # Use next() instead of iterator.next()
            remain -= 1
    return result
...
def TestSite(url):
    ...
    print("Trying: " + url)
    try:
        urllib_request.urlopen(url, timeout=3)
        ...
        if e.code == 404:
            ...
            sys.exit()
    except urllib_error.URLError as g:
...
def PasswordAttempt(user, password, url, thread_no, verbose, debug, agent):
    ...
    try:
        req = urllib_request.Request(url, post.encode(), headers)
